...
def copy_file(name: str, destdir: str, destname: str = ""):
    if not destname:
        destname = name
    shutil.copy(os.path.join(FILES_DIR, name), os.path.join(destdir, destname))
...
def create_distrotest_files(directory: str, config: Dict[str, Any]):
    create_file_from_template(
        "run_distrotest.sh.j2", config, directory, executable=True
    )
...
def create_respdiff_files(directory: str, config: Dict[str, Any]):
    ...
    config["resolvers"] = {
        name: res for name, res in config["resolvers"].items() if "respdiff" in res
    }
    ...
    if config["respdiff_stats"]:  # copy optional stats file
        try:
            shutil.copyfile(
                config["respdiff_stats"], os.path.join(directory, "stats.json")
            )
    ...
    return sorted(
        [
            ...
            for fname in glob.glob(
                os.path.join(TEST_CASE_DIR, "{}*.yaml".format(nameglob))
            )
        ],
...
def main() -> None:
    ...
    parser.add_argument(
        "-t", choices=get_test_case_list(), help="Create only the specified test case"
    )
    parser.add_argument(
        "-l", "--label", help="Assign label for easier job identification and isolation"
    )
    parser.add_argument(
        "--clean",
        action="store_true",
    ...
    parser.add_argument(
        "--asan", action="store_true", help="Build with Address Sanitizer"
    )
    parser.add_argument(
        "--log-keys", action="store_true", help="Log TLS session keys (kresd)"
    )
    parser.add_argument(
        "--respdiff-stats",
        type=str,
        default="",
        help=(
            "Statistics file to generate extra respdiff report(s) with omitted "

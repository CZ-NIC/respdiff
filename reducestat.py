...
def last_jump_of_seq(seq: Sequence[float], window_size: int = WINDOW_SIZE) -> int:
...
def find_last_jump(stats: SummaryStatistics) -> int:
    jumps = []
    jumps.append(last_jump_of_seq(stats.target_disagreements.samples))
    jumps.append(last_jump_of_seq(stats.upstream_unstable.samples))
    jumps.append(last_jump_of_seq(stats.not_reproducible.samples))
    for field in stats.fields:
        jumps.append(last_jump_of_seq(stats.fields[field].total.samples))
    return max(jumps)
...
def cut_field(field: Stats, jump: int) -> Stats:

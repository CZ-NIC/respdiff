        ...
        try:
            ...
            if not(self.kill_flag): 
                ...
                if args.local_port:
                    ...
                    raise IOError("Missing local port name. For example: eth0, tun4... use ifconfig (or ipconfig)")  
                ...
                if args.resolver:
                    if args.resolver in possible_resolvers:
                        ...
                        log.error("Not supported resolver. Possible values are: %s" % ", ".join(map(str, possible_resolvers)))     
                        raise IOError("Not supported resolver. Possible values are: %s" % ", ".join(map(str, possible_resolvers)))    
                else:
...
#                     raise IOError("Missing config file.")      
    ...
    def check_server(self, ip, port):        
        ...
        if result == 0:
            ...
            print("Seems that server just started :] Port is open. Don't forget to kill it after use.")
        else:

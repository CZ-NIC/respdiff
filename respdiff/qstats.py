...
class QueryStatistics(JSONDataObject):
    ...
    @staticmethod
    def from_report(report: DiffReport) -> 'QueryStatistics':
        """Create query statistics for a single report - used to compare against ref"""
        assert report.target_disagreements is not None
        assert report.other_disagreements is not None
        report_failing = set(report.target_disagreements.keys())
        report_unstable = set(report.other_disagreements.queries)
        query_statistics = QueryStatistics()

        for qid in report_failing | report_unstable:
            query_data = report.get_query_data(qid)
            query_statistics.add_query(qid, query_data)
        ...
        for report in reports:
            if report.reprodata is not None:
                ...
                raise NotImplementedError(
                    "Query stability doesn't support diffrepro data when "

...
def add_arg_datafile(parser: ArgumentParser) -> None:
    parser.add_argument(
        ...
        "--datafile",
        type=str,
        help="JSON report file (default: <envdir>/{})".format(REPORT_FILENAME),
    )
...
def add_arg_stats_filename(
    parser: ArgumentParser, default: str = STATS_FILENAME
) -> None:
    parser.add_argument(
        "-s",
        "--stats",
...
def add_arg_report(parser: ArgumentParser) -> None:
    parser.add_argument(
        "report", type=read_report, nargs="*", help="JSON report file(s)"
    )


def add_arg_report_filename(parser: ArgumentParser, nargs="*") -> None:
    parser.add_argument("report", type=str, nargs=nargs, help="JSON report file(s)")
...
def get_datafile(
    args: Namespace, key: str = "datafile", check_exists: bool = True
) -> str:
...
def format_stats_line(
    ...
    *,
    ...
    s["additional"] = (
        "{:30s}".format(additional) if additional is not None else " " * 30
    )
...
def get_stats_data(
    ...
    def percentage(dividend: Number, divisor: Optional[Number]) -> Optional[float]:
...
def get_table_stats_row(
    count: int, total: int, ref_count: Optional[int] = None
) -> Union[StatsTuple, ChangeStatsTupleStr]:
    n, pct, diff, diff_pct = get_stats_data(count, total, ref_count)  # type: ignore
...
def print_fields_overview(
    ...
    for field, counter in field_counters.items():
        ...
        rows.append(
            (field, *get_table_stats_row(field_count, n_disagreements, ref_field_count))
        )
    ...
    for mismatch, count in counter.items():
        ...
        rows.append(
            (
                ...
                *get_table_stats_row(count, n_disagreements, ref_count),
            )
        )
...
def print_global_stats(report: DiffReport, reference: DiffReport = None) -> None:
    ...
    print(
        format_stats_line(
            "duration",
            *get_stats_data(report.duration, ref_n=ref_duration),
            additional="seconds"
        )
    )
    print(
        format_stats_line(
            "queries", *get_stats_data(report.total_queries, ref_n=ref_total_queries)
        )
    )
    print(
        format_stats_line(
            "answers",
            *get_stats_data(
                report.total_answers, report.total_queries, ref_total_answers
            ),
...
def print_differences_stats(report: DiffReport, reference: DiffReport = None) -> None:
    ...
    print(
        format_stats_line(
            "manually ignored",
            *get_stats_data(
                report.summary.manual_ignore, report.total_answers, ref_manual_ignore
            ),
    ...
    print(
        format_stats_line(
            "upstream unstable",
            *get_stats_data(
                report.summary.upstream_unstable,
    ...
    print(
        format_stats_line(
            ...
            *get_stats_data(
                report.summary.not_reproducible,
                report.total_answers,
...
def print_mismatch_queries(
    ...
    if limit is not None and limit < len(queries):
        print(format_line("x", str(len(queries) - limit), "queries omitted"))
    print("")
